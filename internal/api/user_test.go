...
func (ts *UserTestSuite) TestIsDuplicatedEmailWithPlusSuffix() {
	cases := []struct {
		desc     string
		email1   string
		email2   string
		...
		{
			desc:     "Exact same email",
			email1:   "test@example.com",
			email2:   "test@example.com",
			expected: true,
		},
		{
			desc:     "Different plus suffix",
			email1:   "test+suffix1@example.com",
			email2:   "test+suffix2@example.com",
			expected: true,
		},
		{
			desc:     "One with plus suffix, one without",
			email1:   "test@example.com",
			email2:   "test+anything@example.com",
			expected: true,
		},
		{
			desc:     "Different case",
			email1:   "Test+Suffix@Example.com",
			email2:   "test+different@example.com",
			expected: true,
		},
		{
			desc:     "Different base emails",
			email1:   "test1@example.com",
			email2:   "test2@example.com",
			expected: false,
		},
		{
			desc:     "Different domains",
			email1:   "test@example1.com",
			email2:   "test@example2.com",
			expected: false,
		},
	}
	...
	for _, c := range cases {
		ts.Run(c.desc, func() {
			// Create first user
			u1, err := models.NewUser("", c.email1, "password", ts.Config.JWT.Aud, nil)
			require.NoError(ts.T(), err)
			require.NoError(ts.T(), ts.API.db.Create(u1))

			// Try to find duplicate with second email
			dup, err := models.IsDuplicatedEmail(ts.API.db, c.email2, ts.Config.JWT.Aud, nil)
			require.NoError(ts.T(), err)

			if c.expected {
				require.NotNil(ts.T(), dup, "Expected to find duplicate for %s and %s", c.email1, c.email2)
			} else {
				require.Nil(ts.T(), dup, "Expected no duplicate for %s and %s", c.email1, c.email2)
			}
		})
	}
}

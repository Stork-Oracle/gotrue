...
func (a *API) loadSSOProvider(w http.ResponseWriter, r *http.Request) (context.Context, error) {
	...
	if err != nil {
		...
		return nil, notFoundError(apierrors.ErrorCodeSSOProviderNotFound, "SSO Identity Provider not found")
	}
...
func (p *CreateSSOProviderParams) validate(forUpdate bool) error {
	if !forUpdate && p.Type != "saml" {
		return badRequestError(apierrors.ErrorCodeValidationFailed, "Only 'saml' supported for SSO provider type")
	} else if p.MetadataURL != "" && p.MetadataXML != "" {
		return badRequestError(apierrors.ErrorCodeValidationFailed, "Only one of metadata_xml or metadata_url needs to be set")
	} else if !forUpdate && p.MetadataURL == "" && p.MetadataXML == "" {
		return badRequestError(apierrors.ErrorCodeValidationFailed, "Either metadata_xml or metadata_url must be set")
	} else if p.MetadataURL != "" {
		metadataURL, err := url.ParseRequestURI(p.MetadataURL)
		if err != nil {
			return badRequestError(apierrors.ErrorCodeValidationFailed, "metadata_url is not a valid URL")
		}

		if metadataURL.Scheme != "https" {
			return badRequestError(apierrors.ErrorCodeValidationFailed, "metadata_url is not a HTTPS URL")
		}
	}
	...
	default:
		return badRequestError(apierrors.ErrorCodeValidationFailed, "name_id_format must be unspecified or one of %v", strings.Join([]string{
			string(saml.PersistentNameIDFormat),
			string(saml.EmailAddressNameIDFormat),
			string(saml.TransientNameIDFormat),
			string(saml.UnspecifiedNameIDFormat),
...
func parseSAMLMetadata(rawMetadata []byte) (*saml.EntityDescriptor, error) {
	if !utf8.Valid(rawMetadata) {
		return nil, badRequestError(apierrors.ErrorCodeValidationFailed, "SAML Metadata XML contains invalid UTF-8 characters, which are not supported at this time")
	}
	...
	if metadata.EntityID == "" {
		return nil, badRequestError(apierrors.ErrorCodeValidationFailed, "SAML Metadata does not contain an EntityID")
	}

	if len(metadata.IDPSSODescriptors) < 1 {
		return nil, badRequestError(apierrors.ErrorCodeValidationFailed, "SAML Metadata does not contain any IDPSSODescriptor")
	}

	if len(metadata.IDPSSODescriptors) > 1 {
		return nil, badRequestError(apierrors.ErrorCodeValidationFailed, "SAML Metadata contains multiple IDPSSODescriptors")
	}
...
func fetchSAMLMetadata(ctx context.Context, url string) ([]byte, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, badRequestError(apierrors.ErrorCodeSAMLMetadataFetchFailed, "HTTP %v error fetching SAML Metadata from URL '%s'", resp.StatusCode, url)
	}
...
func (a *API) adminSSOProvidersCreate(w http.ResponseWriter, r *http.Request) error {
	...
	if existingProvider != nil {
		return unprocessableEntityError(apierrors.ErrorCodeSAMLIdPAlreadyExists, "SAML Identity Provider with this EntityID (%s) already exists", metadata.EntityID)
	}
	...
	for _, domain := range params.Domains {
		...
		if existingProvider != nil {
			return badRequestError(apierrors.ErrorCodeSSODomainAlreadyExists, "SSO Domain '%s' is already assigned to an SSO identity provider (%s)", domain, existingProvider.ID.String())
		}
...
func (a *API) adminSSOProvidersUpdate(w http.ResponseWriter, r *http.Request) error {
	...
	if params.MetadataXML != "" || params.MetadataURL != "" {
		...
		if provider.SAMLProvider.EntityID != metadata.EntityID {
			return badRequestError(apierrors.ErrorCodeSAMLEntityIDMismatch, "SAML Metadata can be updated only if the EntityID matches for the provider; expected '%s' but got '%s'", provider.SAMLProvider.EntityID, metadata.EntityID)
		}
	...
	for _, domain := range params.Domains {
		...
		if existingProvider != nil {
			if existingProvider.ID == provider.ID {
				...
				return badRequestError(apierrors.ErrorCodeSSODomainAlreadyExists, "SSO domain '%s' already assigned to another provider (%s)", domain, existingProvider.ID.String())
			}
		} else {
			modified = true
	...
	if modified {
		if err := db.Transaction(func(tx *storage.Connection) error {
			...
			return unprocessableEntityError(apierrors.ErrorCodeConflict, "Updating SSO provider failed, likely due to a conflict. Try again?").WithInternalError(err)
		}
	}

...
WORKDIR /go/src/github.com/stork-oracle/gotrue
...
# Building stuff
COPY . /go/src/github.com/stork-oracle/gotrue
...
COPY --from=build /go/src/github.com/stork-oracle/gotrue/auth /usr/local/bin/auth
COPY --from=build /go/src/github.com/stork-oracle/gotrue/migrations /usr/local/etc/auth/migrations/
